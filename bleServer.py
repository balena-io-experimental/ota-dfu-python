...
#------------------------------------------------------------------------------
# Convert an array into a hexidecimal string
#------------------------------------------------------------------------------
...
class BleServer(object):
    ...
    #--------------------------------------------------------------------------
    dfu_control_handle      = 0x1d
    dfu_cccd_handle         = 0x1e

    ota_control_handle      = 0x10
    ota_cccd_handle         = 0x11
    ota_data_handle         = 0x0e
    ...
    #--------------------------------------------------------------------------
    def _dfu_state_set(self, target, opcode):
        self.ble_conn.sendline('char-write-req 0x%04x %04x' % (target, opcode))
    ...
    def _dfu_state_set_byte(self, opcode):
        self.ble_conn.sendline('char-write-req 0x%04x %02x' % (self.ota_control_handle , opcode))
    ...
    def _dfu_pkt_rcpt_notif_req(self):
        ...
        self.ble_conn.sendline('char-write-req 0x%04x %06x' % (self.ota_control_handle , opcode))
    ...
    def _dfu_data_send_req(self, data_arr):
        ...
        #print hex_str
        self.ble_conn.sendline('char-write-req 0x%04x %s' % (self.ota_data_handle , hex_str))
    ...
    def _dfu_data_send_cmd(self, data_arr):
        hex_str = convert_array_to_hex_string(data_arr)
        #print hex_str
        self.ble_conn.sendline('char-write-cmd 0x%04x %s' % (self.ota_data_handle , hex_str))

    #--------------------------------------------------------------------------
    # Enable DFU Control Point CCCD (Notifications)
    #--------------------------------------------------------------------------
    def _dfu_enable_cccd(self, target):
        ...
        self.ble_conn.sendline('char-write-req 0x%04x %s' % (target, cccd_enable_value_hex_string))
    ...
    #--------------------------------------------------------------------------
    def dfu_bootloader(self):

        print "dfu_bootloader"

        # Enable Notifications
        self._dfu_enable_cccd(self.dfu_cccd_handle)

        # Send 'START DFU' + Application Command
        self._dfu_state_set(self.dfu_control_handle, 0x0104)
    ...
    def dfu_send_image(self):
        ...
        # Enable Notifications
        self._dfu_enable_cccd(self.ota_cccd_handle)

        # Send 'START DFU' + Application Command
        self._dfu_state_set(self.ota_control_handle, 0x0104)
        ...
        self._dfu_state_set(self.ota_control_handle, 0x0200)
        ...
        self._dfu_state_set(self.ota_control_handle, 0x0201)
